<script lang="ts">
	// Mob-origin JSON Preview tab — live mob-origin + per-power JSON with AJV
	// validation. Mirrors the player `JsonPreviewTab` but drives off the mob
	// draft, `serializeMobOrigin`, and the `mob_origin.schema.json` schema.
	//
	// Flow:
	//   draft (store) → serializeMobOrigin() → SerializedMobOriginBundle
	//                                        → AJV-validate origin + each power
	//                                        → render path-headed code blocks
	//                                        → list errors with section prefix

	import { base } from '$app/paths';
	import { draft } from '$lib/stores/mobOriginDraft';
	...
	const bundle = $derived(serializeMobOrigin($draft));

	// ── async validation ──────────────────────────────────────────────────
	const bundleKey = $derived(
		JSON.stringify({ o: bundle.mobOrigin, p: bundle.powers.map((p) => p.json) })
	);

	interface ValidationIssue {
		section: string; // e.g. "mob origin" or "power: mypack:flight"
		pointer: string;
		message: string;
		keyword: string;
	}
	...
	async function validateBundle(): Promise<ValidationResult> {
		...
		try {
			const mobValidator = await getValidator(`${base}/schemas/mob_origin.schema.json`);
			const powerValidator = await getValidator(`${base}/schemas/power.schema.json`);

			if (!mobValidator(bundle.mobOrigin)) {
				for (const e of mobValidator.errors ?? []) {
					issues.push(toIssue('mob origin', e));
				}
			}
	...
	async function copy(text: string, key: string): Promise<void> {
		try {
			...
			// Clipboard API can fail in non-secure contexts; user can select-copy.
		}
	}
</script>

<section aria-labelledby="mob-json-heading" class="tab">
	<h2 id="mob-json-heading">JSON Preview</h2>

	{#await validationPromise}
		<div class="status status-loading">Validating...</div>
	{:then result}
		{#if result.errored}
			<div class="status status-warn">Schemas unavailable: {result.errorText}</div>
	...
	<article class="block">
		<header class="block-head">
			<code class="path">{bundle.mobOriginPath}</code>
			<button type="button" class="copy" onclick={() => copy(formatJson(bundle.mobOrigin), 'mob')}>
				{copiedKey === 'mob' ? 'Copied' : 'Copy'}
			</button>
		</header>
		<pre><code>{@html highlight(formatJson(bundle.mobOrigin))}</code></pre>
	</article>
