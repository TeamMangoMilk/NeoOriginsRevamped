...
public sealed interface OriginGate permits OriginGate.HasOrigin, OriginGate.HasPower, OriginGate.InLayer {
    ...
    record HasOrigin(ResourceLocation origin) implements OriginGate {
        ...
        public boolean test(Player player) {
            ...
            for (Map.Entry<ResourceLocation, ResourceLocation> e : data.getOrigins().entrySet()) {
    ...
    record InLayer(ResourceLocation layer, ResourceLocation origin) implements OriginGate {
        public static final MapCodec<InLayer> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
            ResourceLocation.CODEC.fieldOf("layer").forGetter(InLayer::layer),
            ResourceLocation.CODEC.fieldOf("origin").forGetter(InLayer::origin)
        ).apply(inst, InLayer::new));

        public static final StreamCodec<RegistryFriendlyByteBuf, InLayer> STREAM_CODEC =
            StreamCodec.composite(
                ResourceLocation.STREAM_CODEC, InLayer::layer,
                ResourceLocation.STREAM_CODEC, InLayer::origin,
                InLayer::new
            );
        ...
        public boolean test(Player player) {
            PlayerOriginData data = player.getData(OriginAttachments.originData());
            ResourceLocation current = data.getOrigin(layer);
    ...
    Codec<OriginGate> CODEC = ResourceLocation.CODEC.dispatch(
        "type",
        gate -> ResourceLocation.fromNamespaceAndPath("neoorigins", gate.type()),
    ...
    private static MapCodec<? extends OriginGate> codecForType(ResourceLocation id) {
