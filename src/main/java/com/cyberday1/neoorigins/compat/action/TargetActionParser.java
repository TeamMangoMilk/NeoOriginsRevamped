...
public final class TargetActionParser {
    ...
    private static TargetAction parseApplyEffect(JsonObject json) {
        ...
        if (effectOpt.isEmpty()) {
            NeoOrigins.LOGGER.warn("[CompatB] apply_effect (target): unknown mob effect '{}' — action will no-op", effId);
            return (t, a) -> {};
        }
    ...
    private static TargetAction parseClearEffect(JsonObject json) {
        ...
        var effectOpt = BuiltInRegistries.MOB_EFFECT.getOptional(effId);
        if (effectOpt.isEmpty()) {
            NeoOrigins.LOGGER.warn("[CompatB] clear_effect (target): unknown mob effect '{}' — action will no-op", effId);
            return (t, a) -> {};
        }
        var effectHolder = BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effectOpt.get());
    ...
    private static TargetAction parsePlaySound(JsonObject json) {
        ...
        if (soundOpt.isEmpty()) {
            NeoOrigins.LOGGER.warn("[CompatB] play_sound (target): unknown sound event '{}' — action will no-op", sId);
            return (t, a) -> {};
        }
