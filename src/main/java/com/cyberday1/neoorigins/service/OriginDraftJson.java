...
public final class OriginDraftJson {
            ...
            if (o.has("powers") && o.get("powers").isJsonArray()) {
                for (var e : o.getAsJsonArray("powers")) {
                    ...
                    var pd = new OriginDraft.PowerDraft(
                        ResourceLocation.parse(pj.get("powerId").getAsString()),
    ...
    private static ResourceLocation rl(JsonObject o, String k, ResourceLocation def) {
        return o.has(k) ? ResourceLocation.parse(o.get(k).getAsString()) : def;
    }
}
