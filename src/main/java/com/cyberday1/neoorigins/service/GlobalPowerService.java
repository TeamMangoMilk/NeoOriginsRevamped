...
public final class GlobalPowerService {
    ...
    public static void reconcilePlayer(ServerPlayer player) {
        ...
        List<ResourceLocation> desired = GlobalPowerSetDataManager.INSTANCE.playerPowersOrdered();
        boolean changed = false;

        // Revoke powers the ledger owns but that are no longer desired.
        for (ResourceLocation powerId : new ArrayList<>(data.getGlobalGrantedPowers())) {
        ...
        for (ResourceLocation powerId : desired) {
    ...
    private static boolean isProvidedByOrigin(ServerPlayer player, PlayerOriginData data, ResourceLocation powerId) {
