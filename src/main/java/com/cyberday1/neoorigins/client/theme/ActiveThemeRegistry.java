...
public final class ActiveThemeRegistry {
    ...
    private static volatile Identifier serverDeclared; // from SyncActiveThemePayload
    private static volatile Identifier clientOverride; // from neoorigins-client.toml

    /** Server told us which theme its datapack(s) selected (may be null = no declaration). */
    public static void setServerDeclared(Identifier id) {
        serverDeclared = id;
        reapply();
    }
