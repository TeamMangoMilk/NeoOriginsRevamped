...
public class MagicOrbProjectile extends AbstractNeoProjectile {
     ...
     *
     * <p>26.1: {@code Identifier} (not {@code ResourceLocation}) and
     * {@code BuiltInRegistries.PARTICLE_TYPE.get(id)} returns {@code Optional<Holder>}.
     */
    private net.minecraft.core.particles.ParticleOptions resolveTrailParticle() {
        String explicit = getTrailParticle();
        if (explicit != null && !explicit.isEmpty()) {
            var id = net.minecraft.resources.Identifier.tryParse(explicit);
            if (id != null) {
                var ptOpt = net.minecraft.core.registries.BuiltInRegistries.PARTICLE_TYPE.get(id);
                // Only simple (no-data) particle types are usable as ParticleOptions directly.
                if (ptOpt.isPresent()
                        && ptOpt.get().value() instanceof net.minecraft.core.particles.ParticleOptions opt) {
                    return opt;
                }
            }
        }
